...
const Dashboard = () => {
    ...
    // Calculate table stats from mock data
    const stats = useMemo(() => {
        const data = getMockData();
        return {
            totalHires: data.reduce((sum, emp) => sum + emp.hires.thismonth, 0),
            avgAchievement: Math.round(data.reduce((sum, emp) => sum + emp.achieved, 0) / data.length),
            totalSubmittals: data.reduce((sum, emp) => sum + emp.submittals.thismonth, 0),
            avgProductivity: Math.round(data.reduce((sum, emp) => sum + emp.productivity.thismonth, 0) / data.length)
        };
    }, []);
                                    ...
                                    <ComposedChart
                                        ...
                                        data={(() => {
                                            ...
                                            return Object.values(roleData)
                                                .map(data => ({
                                                    role: data.role,
                                                    distribution: Math.round((data.count / filteredData.length) * 100),
                                                    productivity: Math.round(data.productivity / data.count)
                                                }))
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <div className="table-stats">
                                        <span className="table-stat">
                                            ...
                                            <span className="table-stat-value">{stats.totalHires}</span>
                                        </span>
                                        <span className="table-stat">
                                            <span>Avg Achievement:</span>
                                            <span className="table-stat-value">{stats.avgAchievement}%</span>
                                        </span>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <div className="table-stats">
                                        <span className="table-stat">
                                            ...
                                            <span className="table-stat-value">{stats.totalSubmittals}</span>
                                        </span>
                                        <span className="table-stat">
                                            <span>Avg Productivity:</span>
                                            <span className="table-stat-value">{stats.avgProductivity}%</span>
                                        </span>
                                    </div>
                                </div>
                                <span className="badge-integrated">✓ ATS Integrated</span>
                            </div>
                            <div className="table-stats">
            ...
            <div className={`sidebar ${isSidebarCollapsed ? 'collapsed' : ''}`}>
                ...
                <div className="logo-section">
                    <div className="logo-text">
                        <h2>{isSidebarCollapsed ? 'W' : 'We360.ai'}</h2>
                        {!isSidebarCollapsed && <p>Analytics Dashboard</p>}
                    </div>
                </div>
                ...
                <nav className="nav-menu">
                    {[
                        ...
                        { id: 'team', label: 'Team Leaderboard', icon: '🏆' },
                        ...
                        <div className="header-left">
                            <div className="header-title-row">
                                <h1 className="header-title">
                                    {activeTab === 'team' ? 'Team Leaderboard' : `${activeTab.charAt(0).toUpperCase() + activeTab.slice(1)} Dashboard`}
                                </h1>
                                {activeTab === 'recruitment' && <span className="badge-integrated">✓ ATS Integrated</span>}
                            </div>
